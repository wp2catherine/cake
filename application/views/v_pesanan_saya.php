								...
								<tr>
									...
									<td>
										...
										<?php if ($value->status_bayar == 0) { ?>
											<span class="badge badge-warning">No Yet Paid</span>
										<?php } else { ?>
											<span class="badge badge-success">Already Paid</span><br>
											<span class="badge badge-primary">Waiting Verifikasi</span>
										<?php } ?>
									</td>
									<td>
										<?php if ($value->status_bayar == 0) { ?>
											<a href="<?= base_url('pesanan_saya/bayar/' . $value->id_transaksi) ?>" class="btn btn-sm btn-flat btn-primary">Paid</a>
										<?php } ?>
						...
						<table class="table">
							...
							<?php foreach ($dikirim as $key => $value) { ?>
								<tr>
									...
									<td>
										<h5><?= $value->no_resi ?><br>
											<button data-toggle="modal" data-target="#diterima<?= $value->id_transaksi ?>" class="btn btn-primary btn-xs btn-flat">Accepted</button>
										</h5>
									</td>
								</tr>
							<?php } ?>
						...
						<table class="table">
							...
							<?php foreach ($selesai as $key => $value) { ?>
								<tr>
									...
									<td>
										<b>Rp.<?= number_format($value->total_bayar, 0) ?></b><br>
										<span class="badge badge-success">Finish</span><br>
									</td>
									<td>
										<h5><?= $value->no_resi ?><br>
										</h5>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer justify-content-between">
					...
					<a href="<?= base_url('pesanan_saya/diterima/' . $value->id_transaksi) ?>" class="btn btn-primary">Yess</a>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
